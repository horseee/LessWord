...
public class GetImportantWord extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			wordres = SQL.get_important_word_for_user(USER, PASS, DB_URL, name);
			wordnumber = wordres.length  / 3;
			userres = SQL.get_define_word_for_user(USER, PASS, DB_URL, name);
		...
		try {
			...
			try {
				for (int i=0; i<wordnumber; i++) {
					String[] wordinfo = SQL.get_word_info(USER, PASS, DB_URL, wordres[3 * i]);
					statusRes = true;
