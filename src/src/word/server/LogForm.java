...
public class LogForm extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String ResInfo = null;
		try {
			res = SQL.select_get_one(USER, PASS, DB_URL, 0, name);
			if (res == null) {
				res = SQL.select_get_one(USER, PASS, DB_URL, 1, name);
			}
		} catch (Exception e) {
			statusRes = false;
			e.printStackTrace();
		...
		if (res == null || res[0] == null) {
			...
			try {
				review_count = SQL.select_review_count(USER, PASS, DB_URL, name);
